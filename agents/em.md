...
ENG MANAGER checks: Does this involve UI/UX changes (new OR existing)?
    ...
    └─ NO (backend-only, no UI) ───────┤
                            ...
                            Reads design spec + technical spec
                                       ↓
                            [UI work only] EM captures screenshots
                            - Analyzes changed files to determine affected screens
                            - Spawns screenshot-capturer with intelligent targets
...
When Developer completes UI work, orchestrate screenshot capture BEFORE invoking Design-Reviewer:
...
**Step 3: Spawn Screenshot-Capturer**

```
screenshot-capturer, please capture screenshots:
...
**Wait for screenshot-capturer to complete and return paths.**
...
Screenshots: [paths from screenshot-capturer]
...
3. You spawn screenshot-capturer again (targeted re-capture only)
